...
/**
 * Merge property values that have matching keys
 * 
 * @param  {Array} propArray - Original property array
 * @param  {Array} newProp - New Property array
 * @return {Object}
 */
function mergeValues(propArray, newProp) {
	return propArray.map((prop, i) => {
		if (propArray[i].prop === newProp.prop) {
			// TODO: figure out why prop doesn't work here
			return {
				prop: propArray[i].prop,
				value: propArray[i].value + ' ' + newProp.value
			}
		}
	});
}
